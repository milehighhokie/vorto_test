...
func farthestPickCurrent(loadList *[]int, loads map[int]load) (int) {
...
func nearestPickCurrent(loadList *[]int, loads map[int]load) (int) {
...
func randomPickCurrent(loadList *[]int, loads map[int]load) (int) {
...
func groupBunchPickCurrent(loadList *[]int, loads map[int]load) (int) {
...
func findClosestPossibleStop(loadList *[]int, loads map[int]load, currentStop int) (int) {
	// find the closest load from the currentStop
	var minDistPossible=999999.0000000
	var minDistPossibleIdx=-1
	for m:=0; m < len((*loadList)); m++ {
...
func findRandomPossibleStop(loadList *[]int, loads map[int]load, currentStop int) (int) {
	randomIdx := rand.Intn(len((*loadList)))
...
func groupBunchPickPossible(loadList *[]int, loads map[int]load, currentStop int) (int) {
...
func sendTruck(loadList *[]int, loads map[int]load, truck *truck, pickCurrentStop func(*[]int,map[int]load) (int), pickPossibleStop func(*[]int,map[int]load,int) (int), tWG *sync.WaitGroup) () {
	var currentStop int
	var possibleStop int

	currentStop = pickCurrentStop(&(*loadList), loads)

	(*truck).stops = append((*truck).stops,currentStop)
	(*truck).mileage = loads[currentStop].distToPickup + loads[currentStop].deliveryDist + loads[currentStop].distToHome
	for i:=0; i < len((*loadList)); i++  {
		possibleStop = pickPossibleStop(&(*loadList), loads, currentStop)

		possibleNewMileage := (*truck).mileage - loads[currentStop].distToHome + loads[currentStop].distToOthers[possibleStop] + loads[possibleStop].deliveryDist + loads[possibleStop].distToHome
		if possibleNewMileage <= 720 {
			(*truck).stops = append((*truck).stops,possibleStop)
			(*truck).mileage = possibleNewMileage
...
func genericLoadProcessing(minTrucks int, loadList []int, loads map[int]load, trucks *[]truck, pickCurrentStop func(*[]int,map[int]load) (int), pickPossibleStop func(*[]int,map[int]load,int) (int) ) () {
	var tWG sync.WaitGroup

	maxTrucks := len(loadList)
	// we know the minimum number of trucks from totaling the distance between pickup and dropoff points divided by 720
	// it is a crude calculation, but we can send out that many trucks concurrently to start with
	for tl:=0; tl <= minTrucks; tl++ {
		tWG.Add(1)
		go sendTruck(&loadList, loads, &(*trucks)[tl], pickCurrentStop, pickPossibleStop, &tWG)
	}
	...
	var possibleStop int
	deliverLoop:
	for t:=minTrucks + 1; t < maxTrucks; t++ {
		if len(loadList) == 0 {
			break deliverLoop
		}
		currentStop = pickCurrentStop(&loadList, loads)
...
func main() {
	...
	minTrucks := int(math.Ceil(minDist/720))

	wg.Add(1)
	go  genericLoadProcessing(minTrucks, loadList1, loads1, &farThenClosestLoadTrucks, farthestPickCurrent, findClosestPossibleStop)
	farThenClosestLoadScore := calcScore(farThenClosestLoadTrucks)

	//wg.Add(1)
	//go  genericLoadProcessing(minTrucks, loadList2, loads2, &nearestThenBunchTrucks, nearestPickCurrent, groupBunchPickPossible)
	nearestThenBunchScore := 999999.0 // calcScore(nearestThenBunchTrucks)

	wg.Add(1)
	go  genericLoadProcessing(minTrucks, loadList3, loads3, &nearThenClosestLoadTrucks, nearestPickCurrent, findClosestPossibleStop)
	nearThenClosestLoadScore := calcScore(nearThenClosestLoadTrucks)

	wg.Add(1)
	go  genericLoadProcessing(minTrucks, loadList4, loads4, &bunchedFirstLoadTrucks, groupBunchPickCurrent, findClosestPossibleStop)
