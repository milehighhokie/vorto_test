...
func main() {
	...
	closestLoadTrucks, closestLoadScore := make([]truck, len(loadList3)), 0.0 
	farThenClosestLoadTrucks, farThenClosestLoadScore :=  make([]truck, len(loadList1)), 0.0 
	nearThenClosestLoadTrucks, nearThenClosestLoadScore := make([]truck, len(loadList2)), 0.0 

	wg.Add(1)
	go  farThenClosestLoadProcessing(loadList3, loads3, &closestLoadTrucks)
